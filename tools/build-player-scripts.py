...
(function(){{
  ...
  function checkState() {{
    fetch('/api/kou-xia/state')
      ...
      .then(function (data) {{
        // 1. Ensure manual input rows are visible in dynamic mode
        var urows = document.querySelectorAll('.unlock-row');
        urows.forEach(function (el) {{ el.style.display = 'flex'; }});

        // 2. Update hint to show active server connection and prompt for password
        var hints = document.querySelectorAll('.lock-hint');
        hints.forEach(function (el) {{
          el.innerHTML = '🟢 系統已連線，請輸入由 GM 提供的解鎖密碼以開啟此章節。';
        }});
